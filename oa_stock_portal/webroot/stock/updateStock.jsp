...
function init()
{
    ...
    for (var i = 0; i < checkArr.length; i++)
    {
        ...
        else
        {
            ...
            if ($O('productName_' + index)){
                $O('productName_' + index).value = '';
            }

            if ($O('productId_' + index)){
                ...
                <p:cells id="selects" celspan="2" title="采购处理">
                    <table id="mselect">
                        ...
                        <c:forEach items="${bean.itemVO}" var="item" varStatus="vs">
                            <tr>
                                <td>
                                    <input type="checkbox" name="check_init" id="check_init_${vs.index}" value="${vs.index}" onclick="init()">产品${vs.index + 1}：
                                    <input type="button"
                                    ...
                                    <input type="button" value="&nbsp;选 择供应商&nbsp;" name="btn_provider_${vs.index}" class="button_class"
                                           onclick="selectProvider(${vs.index})">&nbsp;
                                    供应商:
                                    <input type="text" name="providerName_${vs.index}" value="${item.providerName}" size="20" readonly="readonly">
                                    <input type="hidden" name="providerId_${vs.index}" value="${item.providerId}">&nbsp;
                                    参考价格:
                                    <input
                                    ...
                                    发票类型:
                                    <select name="invoiceType_${vs.index}">
                                        <option value="">--</option>
                                        <c:forEach items="${invoiceList}" var="invoiceItem">
                                            <c:if test="${invoiceItem.id == item.invoiceType}">
                                                <option value="${invoiceItem.id}" selected>${invoiceItem.name}</option>
                                            </c:if>
                                            <c:if test="${invoiceItem.id != item.invoiceType}">
                                    ...
                                    纳税实体:
                                    <select name="dutyId_${vs.index}">
                                        <option value="">--</option>
                                        <c:forEach items="${dutyList}" var="dutyItem">
                                            <c:if test="${dutyItem.id == item.dutyId}">
                                                <option value="${dutyItem.id}" selected>${dutyItem.name}</option>
                                            </c:if>
                                            <c:if test="${dutyItem.id != item.dutyId}">
